...
class block:
    def __init__(self, ftype):
        assert(ftype == 'd' or ftype == 'f' or ftype == 's' or ftype == 'free')
        self.ftype = ftype
...
class fs:
    ...
    # DONE
        ...
        parent_bnum = self.inodes[parent_inum].getAddr()
        self.inodes[parent_inum].decRefCnt()
        self.data[parent_bnum].delDirEntry(tfile)
    ...
    # DONE
        ...
        parent_inum = self.nameToInum[parent]
        parent_bnum = self.inodes[parent_inum].getAddr()
        if self.data[parent_bnum].getFreeEntries() <= 0:
            return tinum
        if self.data[parent_bnum].dirEntryExists(newfile):
            return tinum
        ...
        self.data[parent_bnum].addDirEntry(newfile, tinum)

        return tinum

    def createSoftLink(self, parent, target, newfile):
        ...
        self.data[bnum].addTarget(target)
        self.inodes[parent_inum].incRefCnt()
        self.data[parent_bnum].addDirEntry(newfile, inum)
    ...
    # DONE
        ...
        if self.data[parent_bnum].dirEntryExists(newfile):
            return inum
        for i in range(self.data[parent_bnum].getNumEntries()):
            assert(self.data[parent_bnum].getEntry(i)[0] != newfile)
        inum = self.inodeAlloc()
        if inum < 0:
            return inum
    ...
    def writeFile(self, tfile, data, iters = 0):
        inum = self.nameToInum[tfile]
    ...
    # DONE
        ...
        if self.inodes[inum].getType() == 'f':
            ...
            assert(self.inodes[inum].getType() == 's')
            dnum = self.inodes[inum].getAddr()
            target = self.data[dnum].getTarget()
            if target not in self.nameToInum:
                return -1
            if (self.inodes[self.nameToInum[target]].getType() == 'free'):
                print 'Softlink to deleted file!'
                return -1
            return self.writeFile(target, data, iters + 1)
    ...
    def run_softlink_test(self):
        ...
        print 'softlink("/u", "/x");'
        self.files.append("/x")
        self.nameToInum['/x'] = self.createSoftLink('/', '/u', 'x')
        print
        self.dump()
        print

        print 'fd=open("/x", O_WRONLY|O_APPEND); write(fd, "77", BLOCKSIZE); close(fd);'
        self.writeFile('/x', "77")
        print
        self.dump()
        print
        ...
        print 'softlink("/x", "/u");'
        self.files.append("/u")
        self.nameToInum['/u'] = self.createSoftLink('/', '/x', 'u')
        print
        self.dump()
        print
