...
setup_user_paging()
{
  ...
  int *usr_pg_tbl = pg_tbl[15] + 1024;
  pg_dir[256] = (int)usr_pg_tbl | PTE_P | PTE_W | PTE_U;
  for (i=0;i<1024;i++) usr_pg_tbl[i] = (USRPHY+(i<<12)) | PTE_P | PTE_W | PTE_U;
}
...
setup_kernel_paging()
{
  ...
  for (i=0;i<16;i++) pg_dir[i+768] = (int)pg_tbl[i] | PTE_P | PTE_W | PTE_U;
  pg_dir[0] = pg_dir[768];
